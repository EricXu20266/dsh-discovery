...
export const zh = {
  ...
  onlyPluginsDesc: '仅显示确认为 DSH 插件的仓库，未确认的自动沉底',
  aiSummaryToggle: '生态摘要注入 system prompt',
  aiSummaryDesc: '开启后，每次与 AI 对话前会自动向 AI 的系统提示注入社区插件生态概况（插件总数、已安装插件、可用工具），让 AI 主动感知插件生态、在你问之前就知道能装什么。代价：每次对话消耗约 200 token。关闭后 AI 不再被动获得生态信息，但仍可用 dsh_discovery_search / dsh_discovery_audit 工具按需查询。',
...
export const en = {
  ...
  onlyPluginsDesc: 'Show only repos verified as DSH plugins; unverified ones sink to the bottom',
  aiSummaryToggle: 'Ecosystem summary → system prompt',
  aiSummaryDesc: 'When ON, before every AI conversation the plugin ecosystem overview (repo count, installed plugins, available tools) is injected into the AI\'s system prompt, so the AI knows what can be installed before you ask. Cost: ~200 tokens per conversation. When OFF the AI gets no passive ecosystem info, but can still query on demand via the dsh_discovery_search / dsh_discovery_audit tools.',
